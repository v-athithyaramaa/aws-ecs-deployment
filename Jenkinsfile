pipeline {
    ...
    stages {
        ...
        stage('Check or Create ECR Repo') {
            steps {
                // Binding credentials using withCredentials step
                withCredentials([usernamePassword(
                    credentialsId: 'aws-ecr-keys', 
    ...
    post {
        always {
            ...
            sh '''
                ...
                docker rmi ${ECR_URI}/${ECR_REPO_NAME}:${IMAGE_TAG} || true
                docker rmi ${ECR_URI}/${ECR_REPO_NAME}:latest || true
            '''
        }
        success {
